...
class Notifier:
    """
        Manages room sessions and members along with message routing
    """
    ...
    async def connect(self, websocket: WebSocket, room_names: List[str] = None):
    ...
    async def request_room(self, websocket: WebSocket, room_names: List[str]):
        for room_name in room_names:
            if self.connections[room_name] == {} or len(self.connections[room_name]) == 0:
                self.connections[room_name] = []
            self.connections[room_name].append(websocket)
